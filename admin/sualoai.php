...
<div id="rightSide"> 
 ...
 $conn = @mysqli_connect("localhost","root","","doan") or die("Không thể kết nối đến sever");
	...
	$sql="SELECT * FROM doan_loai where maLoai='$id'"; //tìm trong bảng có id lấy được bằng get trùng mã sp thì lấy
...
{ 
       ...
       {//kiểm tra xem có nhập thông tin chưa nếu trống thì echo ra
		      ...
		      { $maCL=$_POST['maCL'];
		         if($row['maCL']==$maCL)
				               ...
				               {$add=" update  doan_loai set maCL='$maCL' where maLoai='$id'"   or die("Lỗi truy vấn");
							   $query=mysqli_query($conn,$add);
							   
							   if($query){echo "<META http-equiv=\"refresh\" content=0;URL=http://localhost:8080/do-an/admin/loai.php>";}
       ...
       {//kiểm tra xem có nhập thông tin chưa nếu trống thì echo ra
		              echo '<p style="color:red">Vui lòng không để trống mã loại</p> ';
		}else
		      { $maloai=$_POST['maLoai'];
				...
				echo "<META http-equiv=\"refresh\" content=0;URL=http://localhost:8080/do-an/admin/loai.php>";
				               ...
				               {$add=" update  doan_loai set maLoai='$maLoai' where maLoai='$id'"   or die("Lỗi truy vấn");
							   $query=mysqli_query($conn,$add);
       ...
       {//kiểm tra xem có nhập thông tin chưa nếu trống thì echo ra
		              echo '<p style="color:red">Vui lòng không để trống tên loại</p> ';
		}else
		      { $tenLoai=$_POST['tenLoai'];
		         if($row['tenLoai']==$tenLoai)
			                   {
				 echo "<META http-equiv=\"refresh\" content=0;URL=http://localhost:8080/do-an/admin/loai.php>";
							   }
				else
				               {$add=" update  doan_loai set tenLoai='$tenLoai' where maLoai='$id'"   or die("Lỗi truy vấn");
							   $query=mysqli_query($conn,$add);
       ...
       <div class="formRow">
                <label class="formLeft" for="param_name">Tên loại:<span class="req">*</span></label>
                <div class="formRight"> <span class="oneTwo">
                  <input name="tenLoai" id="param_name" _autocheck="true" type="text"  value="<?php echo $ten;?>"/>
                  </span> <span name="name_autocheck" class="autocheck"></span>
                  <div name="name_error" class="clear error"></div>
                </div>
                <div class="clear"></div>
     ...
     <?php }?>
          ...
          <div class="formSubmit">
            <input type="submit" value="Sửa loại sản phẩm " class="redB" name="submit" />
