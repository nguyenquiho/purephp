      ...
      <li class="product"> <a href="locsp.php" class=" exp" id="current"> <span>Sản phẩm</span> <strong>
	   ...
	   $query3="SELECT * FROM doan_sp ";
        ...
        <ul class="sub">
          ...
          <li class="this"> <a href="admin/cat.html"> Sản phẩm khuyến mãi </a> </li>
          <li > 
...
<div class="wrapper"> 
 ...
 $conn = @mysqli_connect("localhost","root","","doan") or die("Không thể kết nối đến sever");
	...
	$sql="SELECT * FROM doan_sp_km where maSP='$id'"; //tìm trong bảng có id lấy được bằng get trùng mã sp thì lấy
...
{ 
       ...
       {//kiểm tra xem có nhập thông tin chưa nếu trống thì echo ra
		      ...
		      { $maSP=$_POST['maSP'];
		         if($row['maSP']==$maSP)
			                   { echo "<META http-equiv=\"refresh\" content=0;URL=http://localhost:8080/do-an/admin/spkm.php>";
							   }
				else
				               {$add=" update  doan_sp_km set maSP='$maSP' where maSP='$id'"   or die("Lỗi truy vấn");
							   $query=mysqli_query($conn,$add);
							   
							   if($query){echo "<META http-equiv=\"refresh\" content=0;URL=http://localhost:8080/do-an/admin/spkm.php>";}
       ...
       {//kiểm tra xem có nhập thông tin chưa nếu trống thì echo ra
		      ...
		      { $tenSP=$_POST['tenSP'];
		         if($row['tenSP']==$tenSP)
				               ...
				               {$add=" update  doan_sp_km set tenSP='$tenSP' where maSP='$id'"   or die("Lỗi truy vấn");
							   $query=mysqli_query($conn,$add);
       ...
       {//kiểm tra xem có nhập thông tin chưa nếu trống thì echo ra
		      ...
		      { $giaSP=$_POST['giaSP'];
		         if($row['giaSP']==$giaSP)
				               ...
				               {$add=" update  doan_sp_km set giaSP='$giaSP' where maSP='$id'"   or die("Lỗi truy vấn");
							   $query=mysqli_query($conn,$add);
       ...
       {//kiểm tra xem có nhập thông tin chưa nếu trống thì echo ra
		      ...
		      { $thongTin=$_POST['thongTin'];
		         if($row['thongTin']==$thongTin)
				               ...
				               {$add=" update  doan_sp_km set thongTin='$thongTin' where maSP='$id'"   or die("Lỗi truy vấn");
							   $query=mysqli_query($conn,$add);
       ...
       {//kiểm tra xem có nhập thông tin chưa nếu trống thì echo ra
		      ...
		      { $hinhAnh=$_POST['hinhAnh'];
		         if($row['hinhAnh']==$hinhAnh)
				               ...
				               {$add=" update  doan_sp_km set hinhAnh='$hinhAnh' where maSP='$id'"   or die("Lỗi truy vấn");
							   $query=mysqli_query($conn,$add);
       ...
       {//kiểm tra xem có nhập thông tin chưa nếu trống thì echo ra
		      ...
		      { $SL=$_POST['SL'];
		         if($row['SL']==$SL)
				               ...
				               {$add=" update  doan_sp_km set SL='$SL' where maSP='$id'"   or die("Lỗi truy vấn");
							   $query=mysqli_query($conn,$add);
     ...
     <div class="formRow">
      ...
      <div class="formRow">
          ...
          <div class="formSubmit">
            <input type="submit" value="Sửa sản phẩm  " class="redB" name="submit" />
