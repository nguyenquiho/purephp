<?php $conn=mysqli_connect("localhost","root","") or die("Khong the ket noi server!");
			mysqli_select_db($conn,"doan") or die("Database khong ton tai!");
			mysqli_query($conn,"set names 'utf8'");
			$query2="SELECT * FROM doan_sp_moi ";
      ...
      <li class="tran"> <a href="loai.php" class=" exp" > <span>Loại sản phẩm</span> <strong> <?php
	   $query2="SELECT * FROM doan_loai ";
		$data2 = mysqli_query($conn,$query2);
			...
			?></strong> </a> </li>
      <li class="product"> <a href="locsp.php" class="active exp" id="current"> <span>Sản phẩm</span> <strong><?php
...
<div class="titleArea">
  <div class="wrapper">
    ...
    <div class="horControlB menu_action">
      <ul>
        <li><a href="them_locsp.php?&search=<?php	$search = $_GET['search']; echo $search; ?>"> <img src="image/add.png" /> <span>Thêm mới</span> </a></li>
...
<div class="wrapper" id="main_product">
  <div class="widget">
    ...
    <table cellpadding="0" cellspacing="0" width="100%" class="sTable mTable myTable" id="checkAll">
      <thead class="filter">
        <tr>
          <td colspan="9"><form class="list_filter form" action="locsp.php" method="get">
              <table cellpadding="0" cellspacing="0" width="80%">
              <tbody>
                <tr>
                  <td class="label" style="width:70px;"><label for="filter_id">Tên sản phẩm</label></td>
                  <td class="item" style="width:300px;" ><input style="width:300px;" type="text" name="search" placeholder="Tìm kiếm">
            </form></td>
          ...
          <td style='width:150px'><input type="submit" class="button blueB" value="Tìm">
            <input type="reset" class="basic" value="Reset" onclick="window.location.href = 'locsp.php'; "></td>
        </tr>
          </tbody>
    ...
    <?php
		...
		if(isset($_GET['search'])){
			...
			mysqli_select_db($conn,"doan") or die("Database khong ton tai!");
			mysqli_query($conn,"set names 'utf8'");
			$kq=mysqli_query($conn,"select * from doan_sp where thongTin like '%$search%'");//Tìm kiếm bất kỳ giá trị nào mà có $search ở bất kỳ vị trí nào
    ...
    <a href="suasp.php?id=<?php echo $m['maSP'] ?>&search=<?php echo  $search ?> title="Chỉnh sửa" class="tipS">
    <?php	echo		"<img src='image/edit.png' />";
						echo "</a>"; ?>
						
						  <a href="xoasp.php?id=<?php echo $m['maSP'] ?>&search=<?php echo  $search ?>class='tipS'">
					<?php	 echo   "<img src='image/delete.png' />";
   ...
   <center> <p>Trang:   
    ...
    //Lùi 1 nhóm 
		if($gr>1) {?>
      <a href="locsp.php?gr=<?php echo $gr-1;?>&search=<?php echo $search ?>">&lt;&lt;</a>
      <?php } //In ra các trang từ $dau đến $cuoi của nhóm hiện hành: 
    	for($i=$dau;$i<=$cuoi;$i++){
    		if($page==$i){echo "<b><i>$i</b></i> &nbsp";} 
    		else{
    			?>
      <a href="locsp.php?page=<?php echo $i;?>&search=<?php echo $search ?>"><?php echo $i;?>&nbsp</a>
      <?php }}
